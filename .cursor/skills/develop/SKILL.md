...
small-caps `VIRTUAL` + larger Caveat `me` in fixed brand red, regenerated
by `scripts/gen-wordmark.mjs` from pinned TTFs with the exact-pinned
`opentype.js` devDependency),
